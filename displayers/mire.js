class Mire extends StateDisplayer {
  ...
  setPosition() {
    this.x = this.owner.x + this.distanceToOwner*Math.cos(this.angle) + this.size/2*Math.sin(this.angle);
    this.y = this.owner.y + this.distanceToOwner*Math.sin(this.angle) - this.size/2*Math.cos(this.angle);
  }
